...
public class AdminController {
    ...
    public String Registration(Model model, //
        ...
        try {
            ...
            String hashpass1 = accountForm.getEncrytedPassword();
            String hashpass2 = accountForm.getConfirmpassword();
            ...
            if(account != null) {
                ...
                account = new Account();
                if(hashpass1.equals(hashpass2) && hashpass1.matches("((?=.*d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!.#$@_+,?-]).{8,50})")) {
                    account.setUserName(userName);
                    account.setActive(true);
                    account.setUserRole("ROLE_EMPLOYEE");
                    account.setEncrytedPassword(bCryptPasswordEncoder.encode(hashpass1));
    ...
    public String VerifyOTP(Model model, //
                            ...
                            BindingResult result, //
                            final RedirectAttributes redirectAttributes) {
        String otp = accountForm.getOtp();
        if (Utils.validOTP(otp)){
    ...
    public String Resetpassword (Model model, //
                            ...
                            final RedirectAttributes redirectAttributes) {
        String username = accountForm.getUserName();
        String password1 = accountForm.getEncrytedPassword();
        String password2 = accountForm.getConfirmpassword();
        Account account = AccountDAO.findAccount(username);
        ...
        if(password1.equals(password2) && password1.matches("((?=.*d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!.#$@_+,?-]).{8,50})")){
        String hashPwd = bCryptPasswordEncoder.encode(password1);
    ...
    public String ManagerAcc(Model model){

        List<AccountForm> accountForms = this.AccountDAO.getAccounts();
